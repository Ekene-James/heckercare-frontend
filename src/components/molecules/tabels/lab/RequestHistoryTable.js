...
const ModalContent = ({ selectedRow, closeModal }) => {
  ...
  const { mutate: updateStatus, isLoading } = useCustomMutation(
    {
      url: `/lab-stock/fulfill-approved-order/${selectedRow?.id}`,
      method: "patch",
      data: {
        approval: "FULFILLED",
      },
    },
    {
      onSuccess: () => {
        ...
        queryClient.invalidateQueries(GET_ALL_STOCKS);
...
export default function RequestHistoryTable({
  ...
  const { isLoading: searchLoad, refetch: refetchSearch } = useCustomQuery(
    [GET_REQUEST_HISTORY, currentPage, search],
    {
    ...
    },
    {
      ...
      onSuccess: (response) => {
        queryClient.setQueryData(
          [GET_REQUEST_HISTORY, currentPage],
          (oldQueryData) => {
            return {
              ...response,
            };
