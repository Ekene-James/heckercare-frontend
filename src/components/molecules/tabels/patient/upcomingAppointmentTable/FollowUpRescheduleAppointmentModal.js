...
function FollowUpRescheduleAppointmentModal({
  refetch,
  modalContent,
  closeModal,
  currentPage,
  ...
  const {
    ...
    {
      url: `/appointments/reschedule-appointment/${modalContent?.appointment}`,
      method: "patch",
        ...
        <Grid item xs={12} sm={10}>
          <Grid container spacing={1}>
            <Grid item xs={12} sm={4}>
              <CustomTextInput
                title="Patient"
                value={`${modalContent?.patientId?.firstName} ${modalContent?.patientId?.lastName}`}
                disabled="true"
                readOnly
              />
            </Grid>
...
export default FollowUpRescheduleAppointmentModal;
