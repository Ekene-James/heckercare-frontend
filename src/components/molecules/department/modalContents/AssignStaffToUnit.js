...
const AssignStaffToUnit = ({ handleClose, unit, refetch }) => {
  ...
  const {
    ...
    [SEARCH_STAFF, unit?.department?._id, search],
    {
      url: `/department/search-staff/${unit?.department?._id}?search=${search}&limit=1000`,
      method: "get",
  ...
  const { mutate: handleSave, isLoading: addStaffLoading } = useCustomMutation(
    {
      url: `/unit/update-unit/${unit._id}`,
      method: "patch",
      data: {
        staff: [...returnStaffIds([...selectedStaffs, ...unit.staff])],
      },
    },
    {
      onSuccess: () => {
...
export default AssignStaffToUnit;
