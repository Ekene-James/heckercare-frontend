...
const AssignStaffToClinic = ({ handleClose, clinic, refetch }) => {
  ...
  const {
    ...
    [SEARCH_STAFF, clinic?.department?._id, search],
    {
      url: `/department/search-staff/${clinic?.department?._id}?search=${search}&limit=1000`,
      method: "get",
  ...
  const { mutate: handleSave, isLoading: addStaffLoading } = useCustomMutation(
    {
      url: `/clinic/assign-staff/${clinic._id}`,
      method: "patch",
      data: {
        staffId: [...returnStaffIds([...selectedStaffs, ...clinic.staff])],
      },
    },
    {
      onSuccess: () => {
...
export default AssignStaffToClinic;
