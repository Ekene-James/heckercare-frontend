...
import StockRequestModal from "components/molecules/radio/modalContents/StockRequestModal";

import AllStockTable from "components/molecules/tabels/radio/AllStockTable";
import RequestHistoryTable from "components/molecules/tabels/radio/RequestHistoryTable";
...
function RadioStockManagement({ navItems, PageSize, total }) {
  ...
  const { data: stockData, isLoading: stockLoading } = useCustomQuery(
    [GET_RADIOLOGY_STOCKS, currentPage],
    {
      url: `/radiology-stock/?page=${currentPage}&limit=10&search=`,
      method: "get",
    },
    {
  ...
  const { data: history, isLoading } = useCustomQuery(
    [RADIOLOGY_STOCK_HISTORY, currentPage1],
    {
...
RadioStockManagement.defaultProps = {
  navItems: [
    {
      label: "All Stock",
      id: 0,
...
export default RadioStockManagement;
