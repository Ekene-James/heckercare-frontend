...
function StaffRegistration() {
  ...
  const handleNestedSelectDate = (keyName, date, name) => {
    setValues({
      ...values,
      [keyName]: {
        ...values[keyName],
  ...
  const onSave = () => {
    ...
    const data = values?.filename || values?.signature ? formData : values;
    mutate(data);
  };

  return (
    <Box>
      <Typography variant="displayMd">Add New Staff</Typography>
      <Paper sx={{ p: 3, mt: 2 }}>
        <ProfilePicture
          blanckImg={values?.filename}
          onAddImg={(files) =>
