...
function ViewRequisitionModal({ detail: data, closeModal }) {
  ...
  const { mutate: startInspection, isLoading: startInspectionLoading } =
    useCustomMutation(
      {
        url: `/requisition/inspecting/${data._id}`,
        method: "patch",
      },
      {
        onSuccess: () => {
  ...
  const { mutate: updateHodApproval, isLoading: updateHodApprovalLoading } =
    useCustomMutation(
      {
        url: `/requisition/headapproval/${data._id}`,
        method: "patch",
        data: values,
      },
      {
  ...
  const { mutate: receiveRequisition, isLoading: receiveRequisitionLoading } =
    useCustomMutation(
      {
        url: `/product/add-batch/${data._id}`,
        method: "post",
      ...
      },
      {
        onSuccess: () => {
          queryClient.invalidateQueries(GET_ALL_PHARMACY_REQUISITIONS);
          queryClient.invalidateQueries(GET_ALL_PHARMACY_PRODUCTS);
          queryClient.invalidateQueries(GET_ACCOUNTING_REQUISITIONS);
  ...
  //  else if (data.accountApproval === "DECLINED") {
  //   <CustomButton
  //     sx={{
  //       pointerEvents: "none",
  //       ml: 3,
  ...
  const handleReceiveRequisition = () => {
    ...
    goodsReceived.forEach((goods) => {
      if (!goods?.batchNumber) {
        errorFields += 1;
        toast.error(
          `Please input Batch ID value for ${goods.productType.drugName}`
        );
      }
      if (!goods?.sellingPrice) {
        errorFields += 1;
        toast.error(
          `Please input Selling Price value for ${goods.productType.drugName}`
        );
      }
      if (
        goods?.sellingPrice &&
        ...
        errorFields += 1;
        toast.error(
          `Selling price cannot be less than the purchase price  for ${goods.productType.drugName}`
        );
      }
      if (!goods?.purchasePrice) {
        ...
        toast.error(
          `Please input Purchase Price value for ${goods.productType.drugName}`
        );
      }
      if (!goods?.expiryDate) {
        errorFields += 1;
        ...
        toast.error(
          `Please input Expiry Date value for ${goods.productType.drugName}`
        );
      }
      if (!goods?.quantityReceived) {
        ...
        toast.error(
          `Please input Quantity Received value for ${goods.productType.drugName}`
        );
      }
      if (goods?.quantityReceived > goods.quantity) {
        ...
        toast.error(
          `Quantity Received for ${goods.productType.drugName} cannot be higher than the required quantity`
        );
      }
      if (goods?.quantityReceived < goods.quantity) {
        errorFields += 1;
        ...
        toast.error(
          `Quantity Received for ${goods.productType.drugName} cannot be lower than the required quantity`
        );
      }
    });
