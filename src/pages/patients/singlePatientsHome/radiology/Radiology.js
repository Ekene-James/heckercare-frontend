...
import UploadResultModal from "components/molecules/radio/modalContents/UploadResultModal";
  ...
  const {
    ...
    ],
    {
      url: useSanitizeQueryParams(
        `/investigation/get-patient-investigations/${id}?testStatus=COMPLETED&departmentType=RADIOLOGY&endDate=${formatDate(
          date[1]
        )}&startDate=${formatDate(date[0])}`
      ),
      method: "get",
      ...
      <Paper sx={{ width: "100%", p: 2, height: "100%" }}>
        ...
        <Box sx={{ p: { sm: 4, xl: 1 } }}>
          {investigationsLoading ? (
            ...
            <InvestigationsAccordionComponent
              data={investigations}
              from="radiology"
            />
          ) : (
